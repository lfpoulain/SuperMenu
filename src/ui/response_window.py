...
class ResponseWindow(QWidget):
    ...
    def _raise_to_front(self):
        """Présenter la fenêtre avec Qt puis renforcer le focus sous Windows."""
    ...
    def _reinforce_foreground_win32(self):
        """Compléter Qt avec Win32 sans rendre l'affichage dépendant de Win32."""
        try:
            hwnd = int(self.winId())
            ...
            logging.warning(f"Impossible de forcer la fenêtre de réponse au premier plan: {e}")
    ...
    def write_response(self):
        ...
        try:
            text = self.final_response if self.final_response is not None else self.response_text.toPlainText()
            if text:
