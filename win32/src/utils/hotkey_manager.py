...
def _parse_hotkey_to_win32(hotkey):
    ...
    if key_l.startswith("f") and key_l[1:].isdigit():
        function_key_number = int(key_l[1:])
        if 1 <= function_key_number <= _FUNCTION_KEY_COUNT:
            return modifiers, _VK_F1 + function_key_number - 1, ""
                ...
                key_map.update(
                    {
                        ...
                        for number in range(1, _FUNCTION_KEY_COUNT + 1)
                    }
                )
...
class PromptHotkeyManager(QObject):
    """Register the optional direct hotkey configured on each text prompt."""
    ...
    def refresh_hotkeys(self):
        """Rebuild all prompt registrations from persisted settings."""
        ...
        for prompt_id, prompt in ordered_prompts:
            ...
            if error:
                ...
                log(
                    f"Raccourci du prompt '{prompt_id}' invalide: {error}",
                    logging.WARNING,
                )
                continue

            def _callback(selected_prompt_id=prompt_id):
                manager = manager_ref()
                if manager is not None:
                    manager.prompt_hotkey_triggered.emit(selected_prompt_id)

            hotkey_id, register_error = _REGISTRY.register(
                modifiers,
                vk,
                _callback,
            )
            if register_error:
                self._errors[prompt_id] = register_error
                log(
                    f"Impossible d'enregistrer le raccourci '{hotkey}' du "
                    f"prompt '{prompt_id}': {register_error}",
                    logging.WARNING,
                )
                continue

            self._registrations[prompt_id] = hotkey_id

        return not self._errors, dict(self._errors)

    def unregister_hotkeys(self):
        for hotkey_id in self._registrations.values():
            _REGISTRY.unregister(hotkey_id)
